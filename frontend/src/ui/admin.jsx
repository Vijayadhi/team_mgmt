    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card modal-xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="form-grid">
            <FormField label="Team summary"><textarea className="field-input field-textarea" value={reportForm.team_summary || ""} onChange={(event) => setReportForm({ ...reportForm, team_summary: event.target.value })} /></FormField>
            {reportForm.rows?.map((row, index) => <div key={`${row.member_name}-${index}`} className="report-row-card">
              <FormField label="Member"><input className="field-input" value={row.member_name || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, member_name: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
              <FormField label="Activity"><textarea className="field-input field-textarea" value={row.activity_summary || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, activity_summary: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
              <FormField label="Extra work"><textarea className="field-input field-textarea" value={row.extra_work_summary || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, extra_work_summary: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
              <FormField label="Challenges"><textarea className="field-input field-textarea" value={row.challenges_summary || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, challenges_summary: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
              <FormField label="Manager notes"><textarea className="field-input field-textarea" value={row.manager_notes || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, manager_notes: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
              <FormField label="Next week action plan"><textarea className="field-input field-textarea" value={row.next_week_action_plan || ""} onChange={(event) => { const rows = [...reportForm.rows]; rows[index] = { ...row, next_week_action_plan: event.target.value }; setReportForm({ ...reportForm, rows }); }} /></FormField>
            </div>)}
            ...
            <FormField label="Bottleneck risk"><textarea className="field-input field-textarea" value={reportForm.bottleneck_risk || ""} onChange={(event) => setReportForm({ ...reportForm, bottleneck_risk: event.target.value })} /></FormField>
          </div>
        </div>
...
export function AdminApp({ session, pushToast, view, setView, onMenuToggle, taskFocusId, clearTaskFocus }) {
  ...
  const load = async (nextFilters = filters) => {
    ...
    } catch (error) { pushToast("error", "Dashboard error", error.message); } finally { setLoading(false); }
  };
  ...
  if (view === "dashboard") {
    ...
    content = <div className="page-stack">
      ...
      <ReportModal reportForm={reportForm} setReportForm={setReportForm} onClose={() => setReportForm(null)} reportBusy={reportBusy} onSave={async () => {
        setReportBusy(true);
        try { const result = await apiFetch(`/api/admin/reports/${reportForm.id}/save`, { method: "POST", body: JSON.stringify(reportForm) }); pushToast("success", "Report finalized", result.message); await load(); window.open(result.download_url, "_blank", "noopener,noreferrer"); } catch (error) { pushToast("error", "Save failed", error.message); } finally { setReportBusy(false); }
      }} />
    </div>;
  } else {
    content = <div className="page-stack">
